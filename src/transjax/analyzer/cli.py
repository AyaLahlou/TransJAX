...
from transjax.analyzer.analyzer import FortranAnalyzer, create_analyzer_for_project
from transjax.analyzer.config.project_config import ConfigurationManager, create_default_config
...
def analyze_command(args) -> None:
    ...
    if args.config:
        ...
        from transjax.analyzer.config.project_config import load_config
    ...
    try:
        ...
        if summary["circular_dependencies"] > 0:
            print(f"  Warning: Circular dependencies: {summary['circular_dependencies']}")
...
def info_command(args) -> None:
    ...
    if args.list_files:
        ...
        from transjax.analyzer.parser.fortran_parser import FortranParser
...
Examples:
  ...
  transjax-analyze config /path/to/project --template scientific_computing

  # Quick analysis with auto-detection
  transjax-analyze analyze /path/to/project --template auto

  # Show project information
  transjax-analyze info /path/to/project --detect --list-files
        """,
    )
